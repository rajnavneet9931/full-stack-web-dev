...
const { isEmailVerified, clearVerification } = require('./otpController');
...
const createOrder = async (req, res) => {
  try {
    ...
    // 2. OTP Gate — email must be verified first
    if (!isEmailVerified(customerInfo.email)) {
      return res.status(403).json({
        success: false,
        message: 'Email not verified. Please complete OTP verification before paying.',
      });
    }

    // 3. Check stock availability before creating payment
...
const verifyPayment = async (req, res) => {
  try {
    ...
    // 2. Signature valid — re-check OTP
    if (!isEmailVerified(customer.email)) {
      return res.status(403).json({
        success: false,
    ...
    // 6. Clear OTP verification — one-time use only
    clearVerification(customer.email);

    // 7. Send order confirmation email with payment ID (non-blocking)
